...
!!! warning
    There is a known problem with health v8.1.0 that will result in the system being unusable following a successful deployment.
...
If you do not want to use a workspace called `masdev` you **must**  customize the playbook, because it is configured by default to install and configure the Health application in this workspace.
