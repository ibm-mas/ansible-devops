...
To run this role with provider `community` you must first run the [cluster_monitoring role](https://ibm-mas.github.io/ansible-devops/roles/cluster_monitoring/). The grafana dashboard and prometheus monitoring resources depend on the `cluster_monitoring` role.
