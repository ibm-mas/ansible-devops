...
REGISTRY_IMAGE=docker-registry
...
do_image_mirror() {
    ...
    if [[ -z "$(openssl s_client -connect ${TARGET_REGISTRY} -state -nbio 2>&1 | grep -i '^Server certificate')" ]]; then
...
do_image_mirror_single_image() {
    ...
    oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${IMAGE}\" \"${TARGET_REGISTRY}/${image_identifier}\" --max-per-registry=3 --filter-by-os '.*' --insecure ${DRY_RUN}" 
    echo "${oc_cmd}"
    eval ${oc_cmd}
...
do_image_mirror_case_images() {
    ...
    for map_file in ${map_files}; do
        ...
        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" -f \"${map_file}\" --max-per-registry=3 --filter-by-os '.*' --insecure ${DRY_RUN}"
        echo "${oc_cmd}"
        eval ${oc_cmd}
            ...
            if [[ "$(is_valid_olm_catalog_tag ${tag})" == "true" ]]; then
                ...
                if [[ ! -z "${all_tags}" ]]; then
                    ...
                    if [[ ${last_tag} =~ ^${tag_significane} ]]; then
                        ...
                        oc_cmd="oc image mirror -a \"${AUTH_DATA_PATH}/auth.json\" \"${image}@${sha}\" \"${image}:${latest_tag}\" --max-per-registry=3 --filter-by-os '.*'--insecure ${DRY_RUN}"
                        echo "${oc_cmd}"
                        eval ${oc_cmd}
...
process_case_csv_file() {
    ...
    local tmp_csv_file="/tmp/${OC_TMP_PREFIX}_$(date "+%Y%m%d%M%S")/${base_csv}"
