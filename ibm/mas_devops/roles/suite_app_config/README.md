...
Allows the user to add tags into the Liberty Server XML to configure the application according with the user specific needs, by creating a secret with the XML tags and adding the name in the `MAS_FACILITIES_LIBERTY_EXTENSION_XML` var.
...
Allows the user to add a personal AES Encryption password. If no secret is passed, a autogenerated AES Encryption password will take place.

- Optional
- Environment Variable: `MAS_FACILITIES_VAULT_SECRET`
- Default: `""`
- Secret Template
```sh
cat <<EOF | oc create -f -
