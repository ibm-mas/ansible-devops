...
Name of the `ManageWorkspace` Custom Resource that will be targeted to import the new certificates.
...
This defines a local path pointing the certificates definition from a custom file. Sample file definition can be found in `files/manage-certs-sample.yml`.

- **Required** if running as standalone role
- Environment Variable: `MANAGE_CERTIFICATES_FILE_PATH_LOCAL`
- Default: None

### manage_certificates
List of certificate contents to import. Used when invoking this role from another playbook/role.
...
Alias prefix for certificate names. The certificate alias name will be concatenated with this prefix plus auto incremented number.
