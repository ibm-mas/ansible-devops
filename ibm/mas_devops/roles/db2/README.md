...
### masbr_backup_type
Required only if `db2_action` is `backup`. Sets backup type for full backup or incremental backup.  
...
Required only if `db2_action` is `backup` and `masbr_backup_type` is `incr`. The version of the backup to create incremental backup based on.
...
Required only if `db2_action` is `restore`. The version of the backup to be restored from.
