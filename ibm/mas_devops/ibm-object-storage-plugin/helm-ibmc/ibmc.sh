...
if [ "$COMMAND" == "install" ]; then
  ...
  set +e
  if [[ ! -z "$CHARTVERSION" ]]; then
    helm ${PASSTHRU[@]} --version "$CHARTVERSION" --create-namespace --namespace ${RELEASE_NAMESPACE} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set dcname="${DC_NAME}" --set region=${REGION} --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  else
    helm ${PASSTHRU[@]}  --create-namespace --namespace ${RELEASE_NAMESPACE} --set dcname="${DC_NAME}" --set region=${REGION} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  fi
  exit_status=$(echo $?)
  ...
  set +e
  if [[ ! -z "$CHARTVERSION" ]]; then
    helm ${PASSTHRU[@]} --version "$CHARTVERSION" --create-namespace --namespace ${RELEASE_NAMESPACE} --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set region=${REGION} --set dcname="${DC_NAME}" --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  else
    helm ${PASSTHRU[@]} --create-namespace --namespace ${RELEASE_NAMESPACE} --set dcname="${DC_NAME}" --set bucketAccessPolicy="${CONFIG_BUCKET_ACCESS_POLICY}" --set region=${REGION} --set provider="${CLUSTER_PROVIDER}" --set workerOS="${WORKER_OS}" --set platform="${PLATFORM}" --set kubeDriver="${KUBE_DRIVER_PATH}" "${FLAGS[@]}"
  fi
  exit_status=$(echo $?)
  if [[ $exit_status -ne 0 ]]; then
    if [[ $flag -eq 1 ]]; then
